...
class OrderServiceTest {
    ...
    @Mock(strictness = LENIENT) RedisMarketRepository redisMarketRepository;
    @Mock(strictness = LENIENT) OrderEventProducer orderEventProducer;
    @Mock(strictness = LENIENT) RedisOrderRepository redisOrderRepository;
    ...
    void setUp() {
        ...
        given(accountRepository.findByUserIdWithLock(1L)).willReturn(Optional.of(account));
    ...
    class MarketBuy {
        ...
        @Test
        @DisplayName("성공 - 고가 기준으로 예수금을 묶는다")
        ...
        @Test
        @DisplayName("성공 - 고가 데이터 없으면 현재가 * 1.3 기준으로 예수금을 묶는다")
        ...
        @Test
        @DisplayName("실패 - 예수금 부족 시 예외를 던진다")
    ...
    class LimitBuy {
        ...
        @Test
        @DisplayName("성공 - 지정가 * 수량만큼 예수금을 묶고 Redis에 저장한다")
        ...
        @Test
        @DisplayName("실패 - 지정가 미입력 시 예외를 던진다")
        void limit_price_null_throws() {
            var req = new CreateOrderRequest("000660", OrderSide.BUY, OrderType.LIMIT, 2L, null);
    ...
    class SellOrder {
        ...
        @Test
        @DisplayName("성공 - 매도 수량만큼 보유주식 블로킹한다")
        void sell_market_blocks_holding() {
            var req = new CreateOrderRequest("005930", OrderSide.SELL, OrderType.MARKET, 5L, null);
            orderService.createOrder(1L, req);
    ...
    class LimitSell {
        ...
        @Test
        @DisplayName("성공 - 매도 수량만큼 보유주식 블로킹하고 Redis에 저장한다")
        void sell_limit_blocks_holding() {
            var req = new CreateOrderRequest("005930", OrderSide.SELL,
        ...
        @Test
        @DisplayName("실패 - 보유 수량 초과 매도 시 예외를 던진다")
        void sell_without_enough_holding_throws() {
            var req = new CreateOrderRequest("005930", OrderSide.SELL, OrderType.MARKET, 20L, null);
    ...
    class MarketHours {
        ...
        @Test
        @DisplayName("실패 - 장 마감 코드(30) 이면 예외를 던진다")
        ...
        @Test
        @DisplayName("실패 - 장 운영 코드가 null 이면 예외를 던진다")
        void market_code_null_throws() {
    ...
    class CancelOrder {
        ...
        @Test
        @DisplayName("성공 - BUY 취소 시 묶인 예수금이 반환된다")
        void cancel_buy_unblocks_deposit() {
            ...
            Order savedOrder = Order.create(
                    account.getId(), StockCode.of("005930"),
                    OrderSide.BUY, OrderType.MARKET, 1L, null, Money.of(78_000L));
            savedOrder.markSubmitted();

            given(orderRepository.findByIdAndAccountId(any(), any()))
        ...
        @Test
        @DisplayName("성공 - SELL 지정가 취소 시 블로킹된 보유수량이 해제된다")
        void cancel_sell_limit_unblocks_holding() {
            account.processBuy(StockCode.of("005930"), 10L, Money.of(60_000L));
            ...
            assertThat(account.getHoldings().get(0).getBlockedQuantity()).isEqualTo(0L);
            then(redisAccountRepository).should().syncAccountToRedis(account);
            then(redisOrderRepository).should().removeLimitOrder(any(), any(), any());
        }

        @Test
        @DisplayName("실패 - 이미 취소된 주문 취소 시 예외를 던진다")
        void cancel_already_cancelled_throws() {
            Order cancelledOrder = Order.create(
                    account.getId(), StockCode.of("005930"),
