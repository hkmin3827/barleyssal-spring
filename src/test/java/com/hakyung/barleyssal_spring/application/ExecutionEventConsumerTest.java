...
class ExecutionEventConsumerTest {
    ...
    @InjectMocks
    ExecutionEventConsumer consumer;
    @Mock
    ...
    void setUp() throws Exception {
        ...
        submittedOrder = Order.create(1L, StockCode.of("005930"),
                OrderSide.BUY, OrderType.MARKET, 5L, null, Money.of(78_000L));
    ...
    class BuySuccess {
        ...
        void mockSetup() throws Exception {
            given(objectMapper.readValue(anyString(), eq(ExecutionEvent.class))).willReturn(successEvent());
        ...
        @Test
        @DisplayName("성공 - 매수 체결 성공 시 예수금 차감과 보유 주식 추가")
    ...
    class CancelProcessing {
        @BeforeEach
        void mockSetup() throws Exception {
            given(objectMapper.readValue(anyString(), eq(ExecutionEvent.class))).willReturn(cancelEvent());
            given(orderRepository.findById(1L)).willReturn(Optional.of(submittedOrder));
            given(accountRepository.findById(1L)).willReturn(Optional.of(account));
            given(orderRepository.save(any())).willAnswer(inv -> inv.getArgument(0));
            given(accountRepository.save(any())).willAnswer(inv -> inv.getArgument(0));
        ...
        @Test
        @DisplayName("성공 - 취소 이벤트 수신 시 묶여있던 예수금을 다시 반환한다")
        void buy_cancel_unblocks_deposit() throws Exception {
            BigDecimal depositBefore = account.getDeposit();
            consumer.onExecutionEvent(EVENT_JSON, ack);

            assertThat(account.getDeposit()).isGreaterThan(depositBefore);
            then(ack).should().acknowledge();
        }
    }

    @Nested
    @DisplayName("비즈니스 및 인프라 에러 처리")
    class ErrorHandling {

        @Test
        @DisplayName("실패 - 주문을 찾을 수 없는 경우 DLQ로 전송하고 Ack 처리한다")
        void order_not_found_goes_to_dlq() throws Exception {
            ...
            then(dlqService).should().sendToDlq(eq("execution.event"), any(), eq("ORDER_NOT_FOUND"), eq("BUSINESS_ERROR"));
            then(redisOrderRepository).should().rollbackOrderToRedis(any(ExecutionEvent.class));
            then(ack).should().acknowledge();
        }

        @Test
        @DisplayName("실패 - 인프라 일시적 장애(Transient) 발생 시 예외를 던져 재시도를 유도한다")
        void transient_exception_throws_for_retry() throws Exception {
            given(objectMapper.readValue(anyString(), eq(ExecutionEvent.class))).willReturn(successEvent());
            given(orderRepository.findById(1L)).willThrow(new TransientDataAccessException("일시적인 인프라 장애 발생") {
            });
